/* Definindo as classes do padrão Composite */
...
class PrayerComponent {
  ...
  // Método para “achatar” a estrutura em um array de folhas
  flatten() {
    throw new Error("Operação não suportada para este componente");
  }
}
...
class PrayerLeaf extends PrayerComponent {
  ...
  // No caso de uma folha, o flatten() retorna um array contendo ela mesma
  flatten() {
    return [this];
  }
}
...
class PrayerComposite extends PrayerComponent {
  ...
  // O flatten percorre todos os seus filhos e “achata” seus componentes (leaf ou composite)
  flatten() {
    let result = [];
...
class Rosary {
  constructor() {
    ...
    this.sequence = []; // será o array de orações já “achatado”
    this.currentStep = 0;
    this.currentMystery = this.getMystery(this.diaDaSemana);

    // Mesmas definições de mistérios (texto copiado do seu código original)
    this.mysteries = {
      joyful: [
  ...
  // Método que gera a árvore composite das orações e, no final, a “achata”
  generateSequence(mysteryType) {
    // Cria o nó raiz para o Rosário
    const root = new PrayerComposite("Rosário");

    // PARTE INICIAL
    const inicial = new PrayerComposite("Parte Inicial");
    inicial.add(
      new PrayerLeaf(
        "Oferecimento",
        "Divino Jesus, eu vos ofereço este terço que vou rezar, contemplando os mistérios de nossa Redenção. Concedei-me, pela intercessão de Maria, vossa Mãe Santíssima, as graças necessárias para bem rezá-lo e ganhar as indulgências desta santa devoção. (Pode-se acrescentar intenções particulares.) Ofereço-Vos também em reparação aos Corações de Jesus e Maria, nas intenções do Imaculado Coração de Maria, nas intenções do Santo Padre, pela Igreja, pelo clero, famílias, vocações sacerdotais, religiosas, missionárias e leigas, pela paz no mundo e pelo Brasil."
      )
    );
    inicial.add(
      new PrayerLeaf(
    ...
    inicial.add(
      new PrayerLeaf(
        "Credo",
        "Creio em Deus Pai todo-poderoso, Criador do céu e da terra. E em Jesus Cristo, Seu Filho, Nosso Senhor, que foi concebido pelo poder do Espírito Santo, nasceu da Virgem Maria, padeceu sob Pôncio Pilatos; foi crucificado, morto e sepultado; desceu à mansão dos mortos; ressuscitou ao terceiro dia; subiu aos céus; está sentado à direita de Deus Pai todo-poderoso; de onde há de vir para julgar os vivos e os mortos. Creio no Espírito Santo, na Santa Igreja Católica, na comunhão dos santos, na remissão dos pecados, na ressurreição da carne e na vida eterna. Amém."
      )
    );
    inicial.add(
      new PrayerLeaf(
    ...
    inicial.add(
      new PrayerLeaf(
        "Ave Maria (1/3)",
        "A primeira Ave Maria em honra a Deus Pai: \nAve Maria, cheia de graça, o Senhor é convosco; bendita sois vós entre as mulheres e bendito é o fruto do vosso ventre, Jesus. Santa Maria, Mãe de Deus, rogai por nós, pecadores, agora e na hora de nossa morte. Amém."
      )
    );
    inicial.add(
      new PrayerLeaf(
        "Ave Maria (2/3)",
        "A segunda Ave Maria a Deus Filho: \nAve Maria, cheia de graça, o Senhor é convosco; bendita sois vós entre as mulheres e bendito é o fruto do vosso ventre, Jesus. Santa Maria, Mãe de Deus, rogai por nós, pecadores, agora e na hora de nossa morte. Amém."
      )
    );
    inicial.add(
      new PrayerLeaf(
        "Ave Maria (3/3)",
        "A terceira Ave Maria ao Espírito Santo: \nAve Maria, cheia de graça, o Senhor é convosco; bendita sois vós entre as mulheres e bendito é o fruto do vosso ventre, Jesus. Santa Maria, Mãe de Deus, rogai por nós, pecadores, agora e na hora de nossa morte. Amém."
      )
    );
    inicial.add(
      new PrayerLeaf(
    ...
    inicial.add(
      new PrayerLeaf(
        ...
        "Ó meu Jesus, perdoai-nos, livrai-nos do fogo do inferno, levai as almas todas para o céu e socorrei principalmente aquelas que mais necessitam da vossa misericórdia."
      )
    );
    root.add(inicial);

    // PARTE DOS MISTÉRIOS (5 mistérios)
    const capitalize = function (str) {
      return str.charAt(0).toUpperCase() + str.slice(1);
    };

    const mysteriesDescriptions = this.mysteries[mysteryType];

    for (let i = 0; i < 5; i++) {
      const mistOrdinal = this.numerosOrdinais[i + 1];
      const mistTitle = capitalize(mistOrdinal) + " Mistério";
      const misterioComposite = new PrayerComposite(mistTitle);
      // O mistério em si
      misterioComposite.add(
        new PrayerLeaf(mistTitle, mysteriesDescriptions[i])
      );
      // Pai Nosso
      misterioComposite.add(
        new PrayerLeaf(
      ...
      // Grupo das 10 Ave Marias – encapsulado em um composite
      const aveMariaComposite = new PrayerComposite("Ave Maria (10 vezes)");
      for (let j = 1; j <= 10; j++) {
        aveMariaComposite.add(
          new PrayerLeaf(
            "Ave Maria (" + j + "/10)",
            "Ave Maria, cheia de graça, o Senhor é convosco; bendita sois vós entre as mulheres e bendito é o fruto do vosso ventre, Jesus. Santa Maria, Mãe de Deus, rogai por nós, pecadores, agora e na hora de nossa morte. Amém."
          )
      ...
      misterioComposite.add(aveMariaComposite);
      ...
      // Adiciona o grupo do mistério à raiz
      root.add(misterioComposite);
    }
    ...
    finalComposite.add(
      new PrayerLeaf(
        "Agradecimento",
        "Infinitas graças vos damos, soberana Rainha, pelos benefí­cios que recebemos, dignai-vos tomar-nos debaixo de vosso poderoso amparo e alegrar-vos com uma Salve-Rainha."
      )
    );
    finalComposite.add(
      new PrayerLeaf(
        "Salve Rainha",
        "Salve Rainha! Mãe de misericórdia, vida, doçura e esperança nossa, a vós bradamos, os degredados filhos de Eva; a vós suspiramos, gemendo e chorando neste vale de lágrimas. Rogai por nós, Santa Mãe de Deus, para que sejamos dignos das promessas de Cristo. Amém."
      )
    );
    root.add(finalComposite);

    // "Achata" a árvore composite para obter um array linear de folhas,
    // que será usado para a navegação (próximo/anterior)
    this.sequence = root.flatten();
...
/*
O UIController e demais códigos permanecem praticamente os mesmos,
pois após gerar a árvore composite, usamos o método flatten() para obter
um array linear que é usado na navegação (próximo/prev) e exibição. */
...
class UIController {
  ...
  setupEventListeners(rosary) {
    ...
    this.startBtn.addEventListener("click", () => {
      ...
      // Gera a sequência usando o composite e “achata” a árvore para obter um array linear
...
/* –––––––––––––––––––––––––––––––––––– */
// Resto do código permanece semelhante (eventos para hamburger, cookies, etc.)
